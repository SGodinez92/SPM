...
for {set i 4} {$i <= $nStory} {incr i} {
	...
	mass [expr 1000*$i + 100 + $nEleFloor]	[expr $wallP/$g]	[expr $wallP/$g]	0. 0. 0. 0.
	mass [expr 1000*$i + 200 + $nEleFloor] 	[expr $wallP/$g]	[expr $wallP/$g]	0. 0. 0. 0.
}

# Rectangular Wall mass
for {set i 4} {$i <= 11} {incr i} {

	
	if {$i == 11} {
		set wallP [expr 12.5*$ft*7488.*$in2*150.*$pcf*0.5]
	} elseif {$i > 6} {
		set wallP [expr 12.5*$ft*7488.*$in2*150.*$pcf]
	} elseif {$i == 6} {
		set wallP [expr 14.25*$ft*7488.*$in2*150.*$pcf]
	} elseif {$i > 3} {
		set wallP [expr 16.0*$ft*7488.*$in2*150.*$pcf]
	} elseif {$i == 3} {
		set wallP [expr 13.25*$ft*7488.*$in2*150.*$pcf]
	} else {
		set wallP [expr 10.5*$ft*7488.*$in2*150.*$pcf]
	}
		
	mass [expr 1000*$i + 300 + $nEleFloor]	[expr $wallP/$g]	[expr $wallP/$g]	0. 0. 0. 0.
	mass [expr 1000*$i + 400 + $nEleFloor]	[expr $wallP/$g]	[expr $wallP/$g]	0. 0. 0. 0.
	mass [expr 1000*$i + 500 + $nEleFloor]	[expr $wallP/$g]	[expr $wallP/$g]	0. 0. 0. 0.
